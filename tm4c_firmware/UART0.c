...
//------------UART0_Init------------
// Initialize the UART0 for 115,200 baud rate (assuming 50 MHz UART0 clock),
...
//void UART0_Init(){
  SYSCTL_RCGC1_R |= SYSCTL_RCGC1_UART0; // activate UART0
	while((SYSCTL_RCGC1_R & SYSCTL_RCGC1_UART0) == 0){};
  SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOA; // activate port A
	while((SYSCTL_PRGPIO_R & SYSCTL_RCGC2_GPIOA) == 0){};
  UART0_CTL_R &= ~0x0001;                      // disable UART0
  UART0_IBRD_R = 17;                    // IBRD = int(16,000,000 / (16 * 57600)) = int(54.25347222)
  UART0_FBRD_R = 23;                     // FBRD = int(0.25347222 * 64 + 0.5) = 16.72222208
  ...
  UART0_CTL_R |= UART_CTL_RXE|UART_CTL_TXE|UART_CTL_UARTEN;// enable Tx, RX and UART0
...
uint8_t UART0_InChar(void){
  while((UART0_FR_R&UART_FR_RXFE) != 0); // wait until the receiving FIFO is not empty
	//strings = (uint8_t)(UART0_DR_R&0xFF);
